...
const makeAlarm = (overrides: Partial<Alarm> = {}): Alarm => ({
  id: 'test-1', startHour: 7, startMinute: 0, durationMinutes: 30,
  intervalMinutes: 10, snoozeDurationMinutes: 5, days: [1, 2, 3, 4, 5],
...
describe('generateSequence', () => {
  ...
  it('generates correct times', () => {
    ...
    expect(seq[0].hour24).toBe(7); expect(seq[0].minute).toBe(0);
    expect(seq[1].hour24).toBe(7); expect(seq[1].minute).toBe(10);
    expect(seq[2].hour24).toBe(7); expect(seq[2].minute).toBe(20);
    expect(seq[3].hour24).toBe(7); expect(seq[3].minute).toBe(30);
  });
  ...
  it('handles midnight boundary wrap', () => {
    ...
    expect(seq[0].hour24).toBe(23); expect(seq[0].crossesMidnight).toBe(false);
    expect(seq[1].hour24).toBe(0); expect(seq[1].crossesMidnight).toBe(true);
  });
  ...
  it('formats display strings correctly', () => {
    const seq = generateSequence(makeAlarm());
    expect(seq[0].display).toBe('7:00'); expect(seq[0].ampm).toBe('AM');
  });
