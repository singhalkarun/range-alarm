...
beforeEach(() => { act(() => { useAlarmStore.getState().reset(); }); });
...
describe('useAlarmStore', () => {
  ...
  it('adds an alarm', () => {
    const { result } = renderHook(() => useAlarmStore());
    act(() => { result.current.addAlarm({ id: 'test-1', startHour: 7, startMinute: 0, durationMinutes: 30, intervalMinutes: 10, snoozeDurationMinutes: 5, days: [1,2,3,4,5], enabled: true }); });
  ...
  it('updates an alarm', () => {
    const { result } = renderHook(() => useAlarmStore());
    act(() => { result.current.addAlarm({ id: 'test-1', startHour: 7, startMinute: 0, durationMinutes: 30, intervalMinutes: 10, snoozeDurationMinutes: 5, days: [1,2,3,4,5], enabled: true }); });
    act(() => { result.current.updateAlarm('test-1', { startHour: 8 }); });
    expect(result.current.alarms[0].startHour).toBe(8);
  });
  it('deletes an alarm', () => {
    const { result } = renderHook(() => useAlarmStore());
    act(() => { result.current.addAlarm({ id: 'test-1', startHour: 7, startMinute: 0, durationMinutes: 30, intervalMinutes: 10, snoozeDurationMinutes: 5, days: [1,2,3,4,5], enabled: true }); });
    act(() => { result.current.deleteAlarm('test-1'); });
    expect(result.current.alarms).toHaveLength(0);
  });
  it('toggles alarm enabled state', () => {
    const { result } = renderHook(() => useAlarmStore());
    act(() => { result.current.addAlarm({ id: 'test-1', startHour: 7, startMinute: 0, durationMinutes: 30, intervalMinutes: 10, snoozeDurationMinutes: 5, days: [1,2,3,4,5], enabled: true }); });
    act(() => { result.current.toggleAlarm('test-1'); });
