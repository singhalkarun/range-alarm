...
export function useScheduleAlarm() {
  ...
  const saveAlarm = useCallback(async (alarm: Alarm) => {
    ...
    if (existing) { updateAlarm(alarm.id, alarm); } else { addAlarm(alarm); }
  ...
  const toggleAndSchedule = useCallback(async (alarm: Alarm) => {
    ...
    if (toggled.enabled) { await scheduleAlarmSequence(toggled); }
    else { await cancelAlarmNotifications(toggled); }
  }, [toggleAlarm]);
